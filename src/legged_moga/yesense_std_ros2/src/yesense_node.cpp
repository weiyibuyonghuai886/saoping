	...
	: Node("yesense_publisher")
	{
		...
		*((unsigned int *)&yis_out) = 0u;		
		...
		{		
			...
			{
				RCLCPP_INFO(this->get_logger(), "Unable to open port:%s\n",serial_port.c_str());
				return;
			}	
		}
		...
		// =================================================
		timer_ 			= this->create_wall_timer(2ms, std::bind(&YESENSE_Publisher::timer_callback, this));
...
void YESENSE_Publisher::publish_msg(yis_out_data_t *result)
{
	...
	marker_info.lifetime = rclcpp::Duration(10s);
